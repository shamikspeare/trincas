...
export default function DiningSubPage() {
  ...
  if (error) {
    return <main className="w-full bg-white min-h-[60vh] flex items-center justify-center"><div className="text-center"><p className="text-gray-500 mb-4">Failed to load page.</p><Link to="/dining" className="text-indigo-600 underline">Back to Dining</Link></div></main>;
  }
      ...
      <div className="w-full flex flex-col pt-6 pb-12">
        <div className="mx-auto max-w-4xl p-6">
          <div className="text-center mb-8">
            {metadata.titleImg ? (
              <img src={metadata.titleImg} alt={title} className="mx-auto object-contain" style={metadata.titleImgStyle || { height: 'clamp(5rem, 14vw, 7rem)' }} />
            ) : (
              <h1 className="text-3xl font-semibold" style={metadata.headingStyle}>{title}</h1>
            )}
          </div>
          ...
          <div className="space-y-8">
            {sections.length > 0 ? sections.map((section) =>
              section.section_type === 'image' ? (
                section.image_url ? <div key={section.id}><img src={section.image_url} alt={section.alt_text || ''} className="w-full rounded-2xl object-cover" style={{ aspectRatio: '16 / 10' }} loading="lazy" decoding="async" /></div> : null
              ) : (
          ...
          <div className="mt-10 text-center"><Link to="/dining" className="text-indigo-600 underline">Back to Dining</Link></div>
        </div>
      </div>
    </main>
  );
