...
import { fetchPageLayout, MUSIC_TAVERN_SCHEDULE_PAGE_KEY } from "../../lib/pageLayouts";

export default function MusicTavernSchedule() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setData(await fetchPageLayout(MUSIC_TAVERN_SCHEDULE_PAGE_KEY));
    } catch (err) {
      setError(err.message || "Failed to load tavern schedule page");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 flex flex-col justify-between">
      <PageContentLayout
        breadcrumbs={[{ label: "Home", link: "/" }, { label: "Music", link: "/music" }, { label: "Tavern Music Schedule" }]}
        layout={data?.layout}
